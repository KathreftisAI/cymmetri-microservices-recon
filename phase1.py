...
def process_new_document(document):
    ...
    # Constructing dynamic Redis key
    redis_key = f"cymmetri-datascience_sh_recon_{recon_id_value}_{batch_id_value}"
...
def store_in_redis(redis_key, final_app_overdue, logins, id_value, display_name, application_id):
    try:
        ...
        # Serialize lists and dictionaries to JSON strings without backslashes
        logins_json = json.dumps(logins, ensure_ascii=False)
        final_app_overdue_json = json.dumps(final_app_overdue, default=str, ensure_ascii=False)

        # Storing data in Redis
        redis_conn.hset(redis_key, "Cymmetri_logins", logins_json)
        redis_conn.hset(redis_key, "final_app_overdue", final_app_overdue_json)
